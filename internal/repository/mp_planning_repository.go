...
func (r *MPPlanningRepository) FindAllHeadersGroupedApprover(organizationLocationID uuid.UUID, status entity.MPPlaningStatus, approver string, requestorId string) (*entity.MPPlanningHeader, error) {
	...
	if approver != "" || status != "" {
		...
		if approver != "admin" {
			...
			case "manager":
				whereApprover = "approver_manager_id IS NULL"
			case "recruitment":
				whereApprover = "approver_recruitment_id IS NULL"
			case "direktur":
				whereApprover = "recommended_by IS NULL AND approver_manager_id IS NOT"
			default:
				whereApprover = ""
				if requestorId != "" {
